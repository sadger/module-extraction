...
public class MultipleExperiments {
	 ...
	 */
	public File copyDirectoryStructure(File source, String sourceLimit, File destination) {
		...
		if(!targetFile.exists()){
			System.out.println("Making directory: " + target);
			targetFile.mkdirs();
		}
	...
	public static void main(String[] args) throws OWLOntologyCreationException, NotEquivalentToTerminologyException, IOException, OWLOntologyStorageException {
		...
		OWLOntology ont = OntologyLoader.loadOntologyAllAxioms(ModulePaths.getOntologyLocation() + "SharedConceptNames/vivo");
		File sigDirectory = new File(ModulePaths.getSignatureLocation() + "testingnames/vivo");
		
		new MultipleExperiments().runExperiments(ont, new File(ModulePaths.getSignatureLocation() + "/testingnames/vivo"),new SharedNameExperiment(ont));
