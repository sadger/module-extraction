...
public class NewIteratingExtractor implements Extractor {
	...
	private Set<OWLLogicalAxiom> getUnsupportedAxioms(Set<OWLLogicalAxiom> axioms){
		...
		filters.addFilter(new SharedNameFilter(new AxiomStructureInspector(axioms), RemovalMethod.RANDOM));
	...
	public static void main(String[] args) {
		...
		OWLOntology ont = OntologyLoader.loadOntologyAllAxioms(ModulePaths.getOntologyLocation() + "Bioportal/vivo");
		SignatureGenerator gen = new SignatureGenerator(ont.getLogicalAxioms());
		for (int i = 0; i < 100 ; i++) {
			Set<OWLLogicalAxiom> module = new NewIteratingExtractor(ont).extractModule(gen.generateRandomSignature(100));
